...
def analysis(sliceno):
	...
	#   which corresponds to three months of Backblaze data.
	...
	#   on the "cleanmodel" column, since we rely on that in the computations.
	#
	# - Hash partitioning on the "cleanmodel" columns means that each model
	#   does only exist in one slice.  So we do not need to consider other
	#   slices for the per model statistics.
